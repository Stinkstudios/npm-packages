import React from 'react';
import renderer from 'react-test-renderer';
import { mount } from 'enzyme';

import Fullbleed from '../fullbleed';

const component = children =>
  <Fullbleed assetRatio="1:1">{children}</Fullbleed>;

// Content needs to be deterministic for the test to make sense,
// so make this a pure function with inline data-uri
const imgTag = (
  <img
    src="data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs="
    alt="black square"
  />
);

const videoTag = (
  <video controls width={400} height={200}>
    <source
      src="data:video/mp4;base64,"
      type="video/mp4"
    />
    <track kind="captions" src="track.vtt" />
  </video>
);

const h1Tag = <h1>invalid</h1>;

it('renders DOM output correctly', () => {
  const tree = renderer.create(component(imgTag)).toJSON();

  expect(tree).toMatchSnapshot();
});

it('renders <img> tags correctly', () => {
  const tree = mount(component(imgTag));
  expect(tree.find('img').length).toBe(1);
});

it('renders <video> tags correctly', () => {
  const tree = mount(component(videoTag));
  expect(tree.find('video').length).toBe(1);
});

it('renders (does not) <h1> tags correctly', () => {
  expect(() => mount(component(h1Tag))).toThrow(
    `Children must be either an 'img' or 'video' HTML Element, or a 'Video' component.`
  );
});

it("adds 'is-below-ratio' when component's ratio is less than child's", () => {
  const tree = mount(
    <Fullbleed assetRatio="2:1">
      <img alt="mock" />
    </Fullbleed>
  );

  const topLevel = tree.first();

  expect(topLevel.hasClass('is-below-ratio')).toBe(true);
});

it("doesn't add 'is-below-ratio' when component's ratio is greater than child's", () => {
  const tree = mount(
    <Fullbleed assetRatio="1:2">
      <img alt="mock" />
    </Fullbleed>
  );

  const topLevel = tree.first();

  expect(topLevel.hasClass('is-below-ratio')).toBe(false);
});

it("adds 'is-below-ratio' when parent's ratio is less than child's", () => {
  const tree = mount(
    <Fullbleed
      parentSize={{
        width: 10,
        height: 100
      }}
      assetRatio="1:1"
    >
      <img alt="mock" />
    </Fullbleed>
  );

  const topLevel = tree.first();

  expect(topLevel.hasClass('is-below-ratio')).toBe(true);
});

it("doesn't add 'is-below-ratio' when parent's ratio is greater than child's", () => {
  const tree = mount(
    <Fullbleed
      parentSize={{
        width: 10,
        height: 1
      }}
      assetRatio="1:1"
    >
      <img alt="mock" />
    </Fullbleed>
  );

  const topLevel = tree.first();

  expect(topLevel.hasClass('is-below-ratio')).toBe(false);
});
