...
const typeError = type => `Classnames must be a string, got ${type}`;

test('validation of valid classnames', () => {
  expect(validateClassNames({ addClass: 'block__element' }, 'addClass')).toBe(
    null
  );
  expect(
    validateClassNames({ addClass: 'block__element--modifier' }, 'addClass')
  ).toBe(null);
});
